...
int main()
{
	...
	try{
		...
		Cgicc cgi;
      		...
      		form_iterator student_id = cgi.getElement("select_student_id");
      		if(student_id != cgi.getElements().end()) {
      			cout << "Your Searching student_id: " << **student_id << "<br>";
      		}
      		...
      		if(MYSQL *conn=mysql_real_connect(&mysql, "127.0.0.1","root","123456","Cgicc_StudentScoreSystem_Project",0,NULL,CLIENT_FOUND_ROWS)){
				...
				if(student_id1 == row1){
					...
					cout<<"<table border=\"1\" cellpadding=\"10\"><tr><th>Name</th><th>Student ID</th><th>Sex</th><th>Years</th><th>Class</th><th>Dept Name</th><th>Chinese</th><th>Math</th><th>English</th><th>Score</th><th>Credit</th><th>Ranking</th></tr><tr>";
					
					for(int i = 0; i < row_max_num; i++){
						cout<<"<td>"<<row[i]<<"</td>"<<"<br>";
						
					}
					cout<< "</table>" << "[+] Search Success!"<<"<br>";
